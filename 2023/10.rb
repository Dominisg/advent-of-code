...
input.each_with_index{|l, y| $s = Vector[l.index('S'), y] if l.include?('S') }

path = Array.new(input.length) { Array.new(input.first.length) {'.'} }
path.vput($s,'S')

positions = []
DIRS.each{ |dir|
    positions.append([$s + dir, -dir]) if PIPES.fetch(input.vget($s + dir), []).include? -dir
}

step = 1
while positions.map(&:first).uniq.size != 1
    positions.map! { |pos, dir|
        pipe = input.vget(pos)
        path.vput(pos, pipe)
        n_dir = (PIPES[pipe] - [dir]).first
        [pos + n_dir, -n_dir]
    }
...
last = positions.first.first
path.vput(last,input.vget(last))

path.each_with_index do |y, yi|
    out = true
    fliping = '|'
...
enclosed = 0
path.first.length.times do |xi|
    out = true
    fliping = '|'
    path.length.times do |yi|
        x = path[yi][xi] 
        if '-' == x || fliping == x
            out = !out
        elsif ['7', 'F'].include?(x)
