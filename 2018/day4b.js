const solve = (input) =>{
    ...
    map.forEach((object)=>{
        if(results[object.id]=== undefined){
            results[object.id] = new Uint8Array(60);
        }
        for(let i=0; i<60; i++)
        {
            results[object.id][i] +=object.minutes[i];
        }
        });
    ...
    for(let key in results){
        let currmax = Math.max(...results[key]);

        if(currmax > max_val){
            max_val = currmax
            max_id = key;
            max_ind = results[key].indexOf(currmax);
        }
    }

    max_id = max_id.substr(1,max_id.length-1);

    return max_id * max_ind;
}
