...
alg = input.first.chars.map{|c| c == 'L' ? 0 : 1}

nodes = input.last.split("\n")
                  .map{ |l| l.scan(/\w+/) }
                  .map{ |node, l, r| { node => [l,r] } }.inject(&:merge)


path = Proc.new { |start, dest|
    i = 0
    alg.cycle { |s|
        i += 1
        start = nodes[start][s]
        break if dest.include?(start)
    }
...
starts = nodes.select { |key, _| key[2] == 'A' }.keys
d = nodes.select { |key, _| key[2] == 'Z' }.keys
p starts.map { |s| path.(s, d) }.reduce(1, :lcm)
