const solve = (input) =>{
    ...
    for(let i =0; i< array.length; i+=2){
        let rec = {}
        rec['date'] = new Date(array[i])
        rec['command'] = array[i+1]
        data.push(rec)
    }
    data.sort((a,b) =>
    {
     ...
     for(let i =0; i< data.length; i++){
         if(data[i].command.includes('Guard')){
             let id = data[i].command.match(/#[0-9]+ /)[0];
             let j= 1;
            
             while(data[i+j] !== void 0 && data[i+j].command.includes('falls')){
                let rec = []
                rec.id = id
                rec['minutes'] = new Uint8Array(60)
                rec.minutes.fill(null)
               
                rec.day = data[i+j].date;
                 for(let k=data[i+j].date.getMinutes(); k<data[i+j+1].date.getMinutes(); k++){
                     rec.minutes[k]=1;
                 }
                 map.push(rec);
                 j+=2
    ...
    // calc how long they were sleeping

    var results = {};
    map.forEach((object)=>{
        if(results[object.id]===undefined){
            results[object.id]=0;
        }
        results[object.id]+=object.minutes.reduce((a,b)=>a+b)
    })

    var max_id = Object.keys(results).reduce((a, b) => results[a] > results[b] ? a : b);
    var sum = new Int8Array(60);
    sum.fill(0);
    map.forEach((object)=>{
        if(object.id === max_id){
            for(let i=0; i<60; i++)
            {
                sum[i] +=object.minutes[i];
            }
        }
    })

    max_id = max_id.substr(1,max_id.length-1);
    var searched_minute = sum.indexOf(Math.max(...sum));
    
    return parseInt(max_id) * searched_minute;
}
